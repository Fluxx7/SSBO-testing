...
float pier_mosk_spectrum(vec2 k_vec) {
    const float g = 9.81;
    const float A = 8.1e-3;
    const float B = 0.74;
    const float omega_naught = g / (1.026 * windSpeed);
    ...
    float second_term = exp(-B * pow(omega_naught * kRcp, 4.0));
...
void main() {
    ...
    imageStore(baseSpectrum, id, vec4(pier_mosk_spectrum(k_vec), 0.0, 0.0, 1.0));
}