...
layout(rgba32f, set = 1, binding = 0) restrict writeonly uniform image2D baseSpectrum;
...
float jonswap(vec2 IDxy) {
    ...
    float r_val = exp(-(omega_term * omega_term) / (2.0 * sigma * sigma * omega_peak * omega_peak));

    float wind_angle = atan(windDirection.y, windDirection.x);
    float delta = theta - wind_angle;

    // cosine-power spreading
    float spread = pow(max(cos(delta), 0.0), 2.0);
