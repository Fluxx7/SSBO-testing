...
public partial class OptimFftHandler : RefCounted {
	...
	private FSLFile mipShader = FSLFile.FromFile("res://assets/Shaders/Compute/FSL/mipmaps.fsl");
	...
	public void Run(float delta, float time) {
		if (!twiddlesGenned) {
			twiddleGen.Dispatch(texSize/2, 1, 1, new Godot.Collections.Dictionary<StringName, Variant>{{"N", texSize}});
			twiddlesGenned = true;
		}
