...
[Tool]
public partial class ComputeControllerNew : Node2D
{
	...
	private ComputeShader jonswapGen;
	private ComputeShader updateSpectrum;
	private ComputeShader makeMaps;
	...
	public override void _Ready() {
		jonswapGen = new ComputeShader("res://assets/Shaders/Compute/GLSL/JONSWAP/jonswap_gen.glsl");
		updateSpectrum = new ComputeShader("res://assets/Shaders/Compute/GLSL/JONSWAP/update_spectrum.glsl");
		makeMaps = new ComputeShader("res://assets/Shaders/Compute/GLSL/JONSWAP/tessendorf_idft_2d.glsl");
		...
		updateSpectrum.BindTextureParameter("spectrumTexture",
			Callable.From((Texture2Drd tex) => { 
				if (spectrumTexRect != null) {
					spectrumTexRect.Texture = tex;
				}
			}));
		
		makeMaps.AssignUniform("spectrumTexture", 0, 0);
		makeMaps.CreateTexture("displacementMap", _texSize, _texSize, 1, 0);
		makeMaps.CreateTexture("gradientMap", _texSize, _texSize, 1, 1);
		makeMaps.BindTextureParameter("displacementMap",
			Callable.From((Texture2Drd tex) => { 
				if (displaceTexRect != null) {
					displaceTexRect.Texture = tex;
				}
			}));
		makeMaps.BindTextureParameter("gradientMap",
			Callable.From((Texture2Drd tex) => { 
				if (gradientTexRect != null) {
					gradientTexRect.Texture = tex;
				}
			}));
	...
	private void GenerateWaves() {
		...
		makeMaps.Dispatch(_texSize / 16,  _texSize / 16, 1, push_constants);
	}
	...
	private void GenerateJonswap() {
		...
		ComputeShader.SetTexture("gaussianNoise", _texSize, _texSize, gaussian);
